...
-- frame, and re-check Mark of the Wild every couple of seconds.
local tickElapsed = 0
local motwElapsed = 0
local procElapsed = 0
...
-- ============================================================
-- Visibility: only show while playing Balance
-- ============================================================
...
SlashCmdList["BOOMKINBUFFWATCHER"] = function(msg)
    ...
    if msg == "debug" then
        ...
        print("|cff9370DBBoomkinBuffWatcher|r Spec check: " .. tostring(IsBalanceSpec()) .. " | Frame shown: " .. tostring(frame:IsShown()))
        local ok, aura = pcall(C_UnitAuras.GetPlayerAuraBySpellID, MOTW_SPELL_ID)
        if ok then
